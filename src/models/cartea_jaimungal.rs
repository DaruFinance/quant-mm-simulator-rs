...
//!
//! Close cousin of Avellaneda-Stoikov.  Reservation:
//!     r(s, q) = s + (κ - 2q) / (2γ)
//! Spread (uses the AS form for direct comparability):
...
pub struct CarteaJaimungalQuoter {
    pub gamma: f64,
    pub k: f64,
    pub kappa: f64,
    pub horizon_ns: i64,
...
impl CarteaJaimungalQuoter {
    pub fn new(
        gamma: f64, k: f64, kappa: f64, horizon_ns: i64,
        ...
        Self {
            gamma, k, kappa, horizon_ns, size, sigma_floor: 1e-9,
...
impl Quoter for CarteaJaimungalQuoter {
    fn quote(&mut self, book: Option<&Book>, inv: f64, t_ns: i64) -> Vec<Decision> {
        ...
        let r = mid + (self.kappa - 2.0 * inv) / (2.0 * self.gamma);
...
mod tests {
    ...
    fn warmup_no_quotes_until_sigma_ready() {
        let mut q = CarteaJaimungalQuoter::new(0.1, 1.5, 0.0, 1_000_000_000_000, 0.001, 1_000_000_000);
    ...
    #[test]
    fn inv_skews_reservation() {
        let mut q = CarteaJaimungalQuoter::new(1.0, 1.5, 0.0, 1_000_000_000_000, 0.001, 1_000_000_000_000);
        q.quote(Some(&book(0, 100.0, 5.0, 101.0, 5.0)), 0.0, 0);
        let out_flat = q.quote(Some(&book(1_000_000, 100.0, 5.0, 101.0, 5.0)), 0.0, 1_000_000);
        let out_long = q.quote(Some(&book(2_000_000, 100.0, 5.0, 101.0, 5.0)), 1.0, 2_000_000);
        ...
        // → bid shifts down by 1.
        assert!(bid_long < bid_flat - 0.5);
    }

    #[test]
    fn kappa_shifts_reservation_up() {
        let mut q0 = CarteaJaimungalQuoter::new(1.0, 1.5, 0.0, 1_000_000_000_000, 0.001, 1_000_000_000_000);
        let mut qk = CarteaJaimungalQuoter::new(1.0, 1.5, 2.0, 1_000_000_000_000, 0.001, 1_000_000_000_000);
        q0.quote(Some(&book(0, 100.0, 5.0, 101.0, 5.0)), 0.0, 0);
        qk.quote(Some(&book(0, 100.0, 5.0, 101.0, 5.0)), 0.0, 0);
        let out0 = q0.quote(Some(&book(1_000_000, 100.0, 5.0, 101.0, 5.0)), 0.0, 1_000_000);
        let outk = qk.quote(Some(&book(1_000_000, 100.0, 5.0, 101.0, 5.0)), 0.0, 1_000_000);
        let b0 = match &out0[0] { Decision::Maker(q) => q.price, _ => panic!() };
        let bk = match &outk[0] { Decision::Maker(q) => q.price, _ => panic!() };
        assert!(bk > b0);
    }

    #[test]
    fn deterministic_reruns() {
        let mut q1 = CarteaJaimungalQuoter::new(0.5, 1.5, 0.0, 1_000_000_000_000, 0.001, 1_000_000_000_000);
        let mut q2 = CarteaJaimungalQuoter::new(0.5, 1.5, 0.0, 1_000_000_000_000, 0.001, 1_000_000_000_000);
